...
1. Cola cada prompt no Midjourney (um por bloqueio). Começam por
   `same woman serene` de propósito, para reforçar a coerência com a `--cref`.
2. Guarda cada imagem como `public/images/journey/<bloqueio>.jpg` com estes
   nomes exactos: `peso`, `vazio`, `culpa`, `medo`, `vergonha`, `magoa`,
   `rancor`. Aparecem sozinhas no topo de cada bloqueio.
3. A faixa no topo é `3:2`, recortada ao centro. Convém a Vivianne ficar
   centrada.
